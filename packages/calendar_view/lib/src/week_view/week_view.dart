...
class WeekView<T> extends StatefulWidget {
  ...
  /// Display clock start in a day.
  final int clockStart;

  /// Display clock end in a day.
  final int clockEnd;
...
class WeekViewState<T> extends State<WeekView<T>> {
  ...
  void initState() {
    ...
    // clockStart/clockEnd check.
    // debugPrint('[WeekView.initState()]: clockStart=${widget.clockStart}');
    // debugPrint('[WeekView.initState()]: clockEnd=${widget.clockEnd}');
    ...
    _height = _hourHeight * (widget.clockEnd - widget.clockStart);
    _timeLineOffset = widget.timeLineOffset;
    _pageController = PageController(initialPage: _currentIndex);
    _eventArranger = widget.eventArranger ??
        SideEventArranger<T>(); // Default use SideEventArrage
  ...
  Widget build(BuildContext context) {
    // clockStart/clockEnd check.
    // debugPrint('[WeekView.build()]: clockStart=${widget.clockStart}');
    // debugPrint('[WeekView.build()]: clockEnd=${widget.clockEnd}');
    // debugPrint("width in WeekView: $_width");
                  ...
                  child: PageView.builder(
                    ...
                    itemBuilder: (_, index) {
                      ...
                      return InternalWeekViewPage<T>(
                        ...
                        clockStart: widget.clockStart,
                        clockEnd: widget.clockEnd,
                      );
                    },
                  ),
                ),
  ...
  Widget _defaultWeekDayBuilder(DateTime date) {
    // Pochien Kao on 2022/01/14
    // 原weekday第一天為Monday, 使用以下方法變更顯示第一天為Sunday,
    // 另外, constants.dart中的weekTitles要改.
    // final weekday = (date.weekday == 7) ? 0 : date.weekday;
    // debugPrint("$date : date.weekday = ${date.weekday}, weekday = $weekday");
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          // Text(Constants.weekTitles[weekday]),
  ...
  Widget _defaultEventTileBuilder<T>(
    ...
    if (events.isNotEmpty)
      return RoundedEventTile(
        ...
        // padding: EdgeInsets.all(Constants.eventTileSidePadding),
        // margin: EdgeInsets.fromLTRB(Constants.eventTileSidePadding, 0,
        //     Constants.eventTileSidePadding, 0),
