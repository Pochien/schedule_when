...
class EventController<T> extends ChangeNotifier {
  ...
  List<_YearEvent<T>> _events = <_YearEvent<T>>[];
  List<CalendarEventData<T>> _eventList = <CalendarEventData<T>>[];
  List<CalendarEventData<T>> _rangingEventList = <CalendarEventData<T>>[];
  ...
  List<CalendarEventData<T>> get events => _eventList.toList(growable: true);
  ...
  void remove(CalendarEventData<T> event) {
    _eventList.removeWhere((item) => item.eventId == event.eventId);
  ...
  /// Remove all events.
  void removeAll() {
    _events = <_YearEvent<T>>[];
    _eventList = <CalendarEventData<T>>[];
    _rangingEventList = <CalendarEventData<T>>[];
    notifyListeners();
  }
  ...
  List<CalendarEventData<T>> getEventsOnDay(DateTime date) {
    ...
    /// Pochien Kao on 2022/01/13
    for (final event in _eventList) {
      if (event.date == DateTime(date.year, date.month, date.day)) {
        events.add(event);
      }
    }
    // debugPrint("events.length in getEventsOnDay: ${events.length}");
    return events;

    /// Bugs
    /// 應該是因為考慮到誇日的問題, 所以判斷的很複雜.
    ...
    //   }
    // }
    // debugPrint("events.length in getEventsOnDay 1: ${events.length}");
    ...
    //   }
    // }
    // debugPrint("events.length in getEventsOnDay 2: ${events.length}");
